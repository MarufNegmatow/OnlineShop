...
import { ForAddToCart, getCart, GetCategory, GetProducts } from '../api/forProductsApi'
...
const Home = () => {
  ...
  const {products , category} = useSelector((state)=>state.forProductSlice)
  ...
  return (
    <div className="py-[]">
      <div className="section1  py-[70px] flex items-center justify-around sm:flex-col lg:flex-row sm:gap-[50px]">
        <div className="flex sm:flex-wrap sm:gap-5 lg:flex-col lg:w-[20%] ml-[100px]">
          
          {
            category.slice(0, 9).map((elem) => {
              return (
                <Link to={`/category/${elem.id}`} key={elem.id}>
                  <p className="sm:p-[10px_20px] sm:bg-[#F5F5F5] lg:p-0 lg:bg-inherit">
                    {elem.categoryName}
                  </p>
                </Link>
              );
      ...
      <div className="section2">
        <h2 className="flex items-center gap-3 text-[#DB4444] w-[95%] m-auto">
          {" "}
          <span className="bg-[#DB4444] p-[40px_20px]"></span>Today’s
        </h2>
        ...
        <div className="text-center py-[20px] ">
          <Link to={"/category"}>
            <Button className="bg-[#DB4444]  text-white p-[20px_40px]">
              View All Products
            </Button>
          </Link>
        </div>
              ...
              category.slice(0, 6).map((elem) => {
                return (
                  <Link to={`/category/${elem.id}`}
                    key={elem.id}
                    className="group hover:bg-[#DB4444] border-[2px] border-[#91919170] w-[170px] h-[145px] flex flex-col justify-evenly items-center"
                  >
                    <img
                      className="w-[50px] h-[50px] group-hover:filter group-hover:invert group-hover:contrast-100"
                      src={
                        import.meta.env.VITE_APP_FILE_URL + elem.categoryImage
                      }
                      alt=""
                    />
                    <h1 className="text-[16px] text-center     font-[100] group-hover:text-white">
                      {elem.categoryName}                 </h1>
                  </Link>
                );
              })
            }
      ...
      <div className="section5 flex bg-black text-white lg:justify-around items-center sm:flex-col-reverse lg:flex-row my-[20px]">
        <div>
          ...
          <h2 className="text-[32px] py-[20px]">
      ...
      <div className="section6">
        <h2 className="flex items-center gap-3 text-[#DB4444] w-[95%] m-auto">
          {" "}
          <span className="bg-[#DB4444] p-[40px_20px]"></span>Our Products
        </h2>
      ...
      <div className="section7 py-[50px]">
            ...
            <div className="flex overflow-hidden flex-col items-end px-14 max-w-full rounded bg-stone-950 w-[570px] max-md:pl-5">
              <div className="flex relative flex-col items-start pt-36 pb-6 w-full min-h-[284px] max-md:pt-24 max-md:pr-5">
                ...
                <div className="flex relative flex-col max-w-full w-[255px]">
                  ...
                  <div className="flex flex-col mt-4 text-base font-medium text-white w-[81px]">
                    <div>Shop Now</div>
                    <img className="object-contain w-full aspect-[83.33]" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="w-[100%] m-auto flex justify-center items-center py-[100px]">
          <div className="w-[80%] m-auto flex flex-wrap justify-around items-center gap-[20px] mt-[50px]">
